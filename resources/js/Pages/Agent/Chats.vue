          ...
          >
            ...
            <form @submit.prevent="sendReply" v-if="selectedChat?.assigned_agent_id === auth_user.id" class="relative px-4 py-3">
              ...
              <textarea
                ...
                :disabled="selectedChat?.status === 'close'"
                :class="[
                  'flex-1 bg-slate-50 border border-slate-200 border-r-0 rounded-l-xl px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-indigo-400 focus:bg-white transition-colors placeholder-slate-400  overflow-hidden',
                  selectedChat?.status === 'close' ? 'opacity-50 cursor-not-allowed' : ''
                ]"
              ></textarea>
