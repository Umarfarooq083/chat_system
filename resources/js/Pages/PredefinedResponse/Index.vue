...
function confirmDestroy(response) {
    if (confirm(`Are you sure you want to delete "${response.title}"?`)) {
        router.delete(route('predefined-responses.destroy', response.id), {
            preserveScroll: true,
        });
    }
}
...
<template>
    <Head title="Predefined Responses" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    <div class="flex gap-2">
                        <input
                            ...
                            @input="router.get(route('predefined-responses.index'), { search: search }, { preserveState: true })"
                            class="sm:w-64 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        />
                    </div>
                    <Link :href="route('predefined-responses.create')"
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="response in responses.data" :key="response.id">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span
                                            :class="response.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            <Link
                                                :href="route('predefined-responses.edit', response.id)"
                                                class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                            >
                                                Edit
                                            </Link>
                                            <button
                                                @click="confirmDestroy(response)"
                                ...
                                <tr v-if="!responses.data.length">
                                    <td colspan="5" class="px-6 py-10 text-center text-sm text-gray-500">
                                        ...
                                        <template v-else>
                                            No responses found. <Link :href="route('predefined-responses.create')" class="text-indigo-600 hover:underline">Create one</Link>
                                        </template>
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div class="mt-6" v-if="responses.data.length">
                    <div class="flex items-center justify-between text-sm text-gray-600">
                        <span>
                            Showing {{ responses.from }} to {{ responses.to }} of {{ responses.total }} results
                        </span>
                        <div class="flex gap-2">
                            <button
